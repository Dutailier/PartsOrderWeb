...
class Logger
{
    ...
    public function __construct($path)
    {
        ...
        if (!is_dir($path)) {
            trigger_error("<code>$path</code> doesn't exists.", E_USER_WARNING);
            return false;
        }
    ...
    public function path($type, $name, $granularity = self::GRAN_YEAR)
    {
        ...
        if (!isset($type) || empty($name)) {
            trigger_error("parameters incorrect.", E_USER_WARNING);
            return false;
        }
        ...
        if ($granularity == self::GRAN_VOID) {
            ...
            trigger_error("Granularity '$granularity' doesn't exists.", E_USER_WARNING);
            return false;
        }
    ...
    public function log($type, $name, $row, $granularity = self::GRAN_YEAR)
    {
        ...
        if ($logfile === false) {
            trigger_error("Unable to write the log.", E_USER_WARNING);
            return false;
        }
